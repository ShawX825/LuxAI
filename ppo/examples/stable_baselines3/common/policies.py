...
class ActorCriticPolicy(BasePolicy):
    ...
    def evaluate_actions(self, obs: th.Tensor, actions: th.Tensor) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        indices_1 =  obs['unit_info'][:, :2].sum(1) > 0
        # indices_2 =  obs[:, 2] == 1.0
        
        obs_1 = {
            "map": obs['map'][indices_1], 
            "unit_info": obs['unit_info'][indices_1],
            "size": obs['size'][indices_1],
            "pos": obs['pos'][indices_1],
            }
        # obs_2 = {
        #     "map": obs['map'][indices_2], 
        #     "unit_info": obs['unit_info'][indices_2],
        #     "size": obs['size'][indices_2],
        #     }
        
        latent_pi_1, latent_vf_1, latent_sde_1 = self._get_latent(obs_1)
        # latent_pi_2, latent_vf_2, latent_sde_2 = self._get_latent(obs[indices_2])
        
        values_1 = self.value_net(latent_vf_1)
        # values_2 = self.value_net_2(latent_vf_2)
        distribution_1 = self._get_action_dist_from_latent(latent_pi_1, latent_sde_1)
        # distribution_2 = self._get_action_dist_from_latent_2(latent_pi_2, latent_sde_2)
        log_prob_1 = distribution_1.log_prob(actions[indices_1])
        # log_prob_2 = distribution_2.log_prob(actions[indices_2])
        
        # values = th.zeros((obs['unit_info'].shape[0], values_1.shape[1]), device=values_1.device)
        # values[indices_1] = values_1
        # values[~indices_1] = values_2
        
        # log_prob = th.zeros((obs['unit_info'].shape[0]), device=values_1.device)
        # log_prob[indices_1] = log_prob_1
        # log_prob[~indices_1] = log_prob_2
        
        # dist = th.zeros((obs['unit_info'].shape[0]), device=values_1.device)
        # dist[indices_1] = distribution_1.entropy()
        # dist[~indices_1] = distribution_2.entropy()
        
        # print('distribution_1.sample:', distribution_1.sample()[:min(100, distribution_1.sample().shape[0])])
        # print('distribution_2.sample:', distribution_2.sample())
        
        return values_1, log_prob_1, distribution_1.entropy(), indices_1
